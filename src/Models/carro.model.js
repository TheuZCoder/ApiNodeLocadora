...
const getCarroById = async (id) => {
  try {
    const { rows } = await db.query("SELECT * FROM carro WHERE id_carro = $1", [
      id,
    ]);
    return rows[0];
  } catch (error) {
...
const updateCarro = async (
  ...
  try {
    const { rows } = await db.query(
      "UPDATE carro SET placa_carro = $1, modelo_carro = $2, marca_carro = $3, ano_carro = $4, cor_carro = $5, disponibilidade_carro = $6, img_carro= $7, preco_carro= $8 WHERE id_carro = $9 RETURNING *",
      [
        placa_carro,
        modelo_carro,
        marca_carro,
...
const deleteCarro = async (id) => {
  try {
    const { rows } = await db.query(
      "DELETE FROM carro WHERE id_carro = $1 RETURNING *",
      [id]
    );

    await updateDisponibilidade(id.id_carro, true);
...
const updateDisponibilidade = async (id_carro, disponibilidade_carro) => {
  try {
    const { rows } = await db.query(
      "UPDATE carro SET disponibilidade_carro = $1 WHERE id_carro = $2 RETURNING *",
      [disponibilidade_carro, id_carro]
    );
    return rows[0];
  } catch (error) {
    console.log("Erro durante atualização de carro: ", error);
